...
# Python source
c <- read.csv("./data_random/Old/inyo_north_python.csv", header = TRUE, sep=",")
...
# GEE source
d <- read.csv("./data_random/Old/inyo_north_gee.csv", header = TRUE, sep=",")
...
for (b in bands){
  ...
  grobs[[i]] <- ggplot(df, aes_string(x='x',y='y')) +
	  ...
	  theme_bw()
  ggsave(paste('./plots/random/nofilter_',b,".pdf", sep=''), width = 4, height = 4, dpi = 220)
  i = i+1
}