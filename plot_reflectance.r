...
# Python sources
coord_reflectance <- read.csv("./data_random/inyo_north_python.csv", header = TRUE, sep=",")
...
coord_band_mean <- DT[,.(ref_mean = mean(Reflectance)), by = .(Label,Wavelength,Band)] # Get mean of each band per location
coord_band_mean <- dcast(coord_band_mean, Label ~ Band , ) # Show std per band per label
coord_band_std <- DT[,.(ref_std = sd(Reflectance)), by = .(Label,Wavelength,Band)] # Get std of each band per location
coord_band_std <- dcast(coord_band_std, Label ~ Band , ) # Show std per band per label

print(coord_band_mean)

# 
# # With cloud mask
# coord_reflectance_cloud <- read.csv("./lc_cloudmask.csv", header = TRUE, sep=",")
# 
...
#   bp <- barplot(as.matrix(x1), ylim = range(0:40), beside = TRUE)
